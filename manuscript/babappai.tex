...
Calibration in \texttt{ceii\_v3.1} is two-stage. Stage 1 is an applicability function $A(x)$ over support variables only (minimum: taxa count and gene length; optional branch-count and missingness support variables):
\[
...
\[
c\mathrm{EII}_{\mathrm{gene}}=P(I_{\mathrm{gene}}=1\mid S_{\mathrm{gene}},A_{\mathrm{gene}}=\texttt{in\_domain},\texttt{ceii\_v3.1}),
\]
\[
c\mathrm{EII}_{\mathrm{site}}=P(I_{\mathrm{site}}=1\mid S_{\mathrm{site}},A_{\mathrm{site}}=\texttt{in\_domain},\texttt{ceii\_v3.1}).
\]
...
$c\mathrm{EII}_{\mathrm{gene}},c\mathrm{EII}_{\mathrm{site}}$ & Conditional calibrated $P(I=1\mid\text{evidence},A=\texttt{in\_domain},\texttt{ceii\_v3.1})$; otherwise abstain ($\varnothing$). \\
...
Raw EII remained a useful continuous dispersion score but was not directly interpretable as identifiability probability. Under the revised \texttt{ceii\_v3.1} path, calibrated output is conditional on both applicability support and numerically valid neutral dispersion, and is constrained by an evidence-monotone calibration map. The legacy \texttt{ceii\_v1} behavior that emitted a flat fallback value is no longer used for inference claims; in the revised runtime, unsupported inputs receive raw EII + matched-neutral significance with explicit \texttt{calibration\_unavailable} metadata rather than forced numeric cEII.
