...
Raw EII remained a useful continuous dispersion score but was not directly interpretable as identifiability probability. Under the \texttt{ceii\_v2} design, calibrated output is conditional on applicability support. The previous \texttt{ceii\_v1} behavior emitted a flat numeric fallback on this empirical panel, whereas \texttt{ceii\_v2} explicitly abstains outside support and reports diagnostics. In the replayed Drosophila panel, \texttt{ceii\_v2} produced calibrated gene-level probabilities for 5/8 successful genes and abstained for 3/8 (\texttt{out\_of\_domain} or \texttt{near\_boundary}); raw EII and $q_{\mathrm{emp}}$ remained available for all successful genes.

\subsection{Reliability and operating characteristics}

On held-out test data, gene-scale calibration yielded Brier $=0.202$ and ECE $=0.219$; OOD values were Brier $=0.479$ and ECE $=0.514$. Site-scale calibration was less stable (test: Brier $=0.014$, ECE $=0.073$; OOD: Brier $=0.201$, ECE $=0.195$), with marked PPV/FDR degradation in low-prevalence strata. These quantitative results support the intended interpretation: gene-level identifiability can emerge before site-level localization becomes decision-stable, and OOD applicability must be surfaced rather than hidden.

\subsection{Empirical applicability and abstention behavior}

On the Drosophila control panel, the historical \texttt{ceii\_v1} asset returned a flat gene-level value ($0.625$) for all successful genes. Replaying the same raw outputs under the abstention-aware runtime showed that only 1/8 successful genes was in-domain for \texttt{ceii\_v1}, so 7/8 should have abstained instead of receiving a numeric probability. Under expanded \texttt{ceii\_v2} support, 5/8 successful genes were in-domain, 1/8 was near-boundary, and 2/8 were out-of-domain. This behavior is intentional: calibrated probabilities are emitted only where validated support exists; otherwise BABAPPAi reports raw EII + matched-neutral significance with explicit \texttt{calibration\_unavailable} status.
