...
\begin{abstract}
Branch--site analyses are often interpreted as evidence for episodic positive selection, but a prior question is whether episodic structure is \emph{recoverable} from the available alignment. BABAPPAi targets this identifiability question and separates it from adaptation claims while using a fixed neural inference backbone: the canonical frozen BABAPPA$\Omega$ model artifact. This work locks the raw dispersion statistic as $D_{\mathrm{obs}}=\mathrm{Var}_{\mathrm{ddof}=1}(W_{0.05}(\overline{\ell}_i))$, where $\overline{\ell}_i$ is the site-level mean branch logit and $W_{0.05}$ denotes two-sided 5\% winsorization, and defines raw $ \mathrm{EII}_z=(D_{\mathrm{obs}}-\mu_0)/\sigma_0 $ under matched-neutral Monte Carlo calibration. A conditional calibration layer is further implemented, $c\mathrm{EII}_{\mathrm{gene/site}}=P(I=1\mid S,A=\text{applicable},\texttt{ceii\_v3.2})$, with strict abstention outside validated applicability support. However, under the current held-out calibration benchmark, applicable-case reliability remains insufficiently stable for cEII to serve as a central manuscript endpoint. Accordingly, the primary claims are based on raw EII and matched-neutral significance ($p_{\mathrm{emp}},q_{\mathrm{emp}}$) with explicit applicability-aware abstention, while cEII is retained as an auxiliary, versioned post-inference diagnostic. This yields a stable fixed-model inference stack with honest uncertainty handling that complements, but does not replace, classical branch--site hypothesis tests.
\end{abstract}
...
\begin{itemize}
    ...
    \item The software reports raw dispersion diagnostics ($\mathrm{EII}_{z,\mathrm{raw}}$, $\mathrm{EII}_{01,\mathrm{raw}}$) and matched-neutral significance universally; cEII is currently treated as an auxiliary, applicability-gated calibration layer.
...
One dispersion definition is locked for all validation and release outputs:
\[
...
To separate ``signal exists'' from ``signal recoverable'', truth-aware recoverability targets are defined on held-out simulations:
\[
...
Recoverability-only binary labels are
\[
I_{\mathrm{gene}}^{\mathrm{rec}}=\mathbf{1}[R_{\mathrm{gene}}\ge \tau_{\mathrm{gene}}], \quad
\tau_{\mathrm{gene}}=0.42,
\]
\[
I_{\mathrm{site}}^{\mathrm{rec}}=\mathbf{1}[R_{\mathrm{site}}\ge \tau_{\mathrm{site}}], \quad
\tau_{\mathrm{site}}=0.45.
\]
Final calibration targets additionally require nontrivial excess over matched-neutral expectation:
\[
G_{\mathrm{excess}}=\mathbf{1}\!\left[q_{\mathrm{emp}}\le q^\star \ \wedge \
\left(\mathrm{dispersion\_ratio}\ge r^\star \ \vee\ \mathrm{EII}_{01,\mathrm{raw}}\ge e^\star\right)\right],
\]
\[
I_{\mathrm{gene}}=I_{\mathrm{gene}}^{\mathrm{rec}}\cdot G_{\mathrm{excess}}, \qquad
I_{\mathrm{site}}=I_{\mathrm{site}}^{\mathrm{rec}}\cdot G_{\mathrm{excess}}.
\]
Gate constants $(q^\star,r^\star,e^\star)$ are derived from neutral-control distributions in the training split only.

Calibration in \texttt{ceii\_v3.2} is two-stage. Stage 1 is an applicability function $A(x)$ over support variables only (minimum: taxa count and gene length; optional branch-count and missingness support variables):
\[
A(x)\in\{\texttt{in\_domain},\texttt{near\_boundary},\texttt{out\_of\_domain}\}.
\]
Only \texttt{in\_domain} samples are calibrated by default.

Stage 2 uses evidence variables only:
\[
z=\big(\mathrm{EII}_{01,\mathrm{raw}},\ \mathrm{clip}(\mathrm{EII}_{z,\mathrm{raw}}),\ -\log_{10}(q_{\mathrm{emp}}+\epsilon),\ \mathrm{dispersion\_ratio}\big),
\]
\[
S=\sum_j w_j f_j(z_j), \qquad w_j\ge 0,
\]
followed by isotonic calibration on held-out calibration data:
\[
c\mathrm{EII}_{\mathrm{gene}}=P(I_{\mathrm{gene}}=1\mid S_{\mathrm{gene}},A_{\mathrm{gene}}=\texttt{in\_domain},\texttt{ceii\_v3.2}),
\]
\[
c\mathrm{EII}_{\mathrm{site}}=P(I_{\mathrm{site}}=1\mid S_{\mathrm{site}},A_{\mathrm{site}}=\texttt{in\_domain},\texttt{ceii\_v3.2}).
\]
Support variables determine whether calibration is allowed; evidence variables determine the probability.
...
\begin{table}[t]
\centering
\caption{Core symbols and locked definitions used in this manuscript.}
...
$I_{\mathrm{gene}},I_{\mathrm{site}}$ & Binary identifiability labels requiring recoverability criteria and excess-over-neutral gate. \\
$A(x)$ & Applicability function for calibrated output validity (\texttt{in\_domain}/\texttt{near\_boundary}/\texttt{out\_of\_domain}). \\
$c\mathrm{EII}_{\mathrm{gene}},c\mathrm{EII}_{\mathrm{site}}$ & Conditional calibrated $P(I=1\mid\text{evidence},A=\texttt{in\_domain},\texttt{ceii\_v3.2})$; otherwise abstain ($\varnothing$). \\
...
Raw EII remained a useful continuous dispersion score but was not directly interpretable as identifiability probability. Under the revised \texttt{ceii\_v3.2} path, calibrated output is conditional on both applicability support and numerically valid neutral dispersion, and is constrained by an evidence-monotone calibration map. The legacy \texttt{ceii\_v1} behavior that emitted a flat fallback value is no longer used for inference claims; unsupported inputs receive raw EII + matched-neutral significance with explicit \texttt{calibration\_unavailable} metadata rather than forced numeric cEII. On the fixed 8-gene diagnostic subset, this removed the prior contradiction where strong in-domain genes collapsed to zero cEII.

\subsection{Reliability and operating characteristics}

For \texttt{ceii\_v3.2}, held-out calibration artifacts (\texttt{ceii\_reliability.tsv}, \texttt{ceii\_split\_performance.tsv}) were generated with full sigma-valid coverage (fraction sigma-valid $=1.0$; sigma-floor collapse $=0.0$). However, the current held-out applicable splits are label-sparse (test/OOD positive rates for $I_{\mathrm{gene}}$ and $I_{\mathrm{site}}$ were near zero in this benchmark instance), which limits interpretability of probability reliability as a primary endpoint. These reliability diagnostics are therefore treated as provisional and cEII is not elevated to the main claim in this manuscript revision.
...
\subsection{All-or-none cEII decision}

Applying a strict all-or-none acceptance rule to the fixed diagnostic subset (ago1, ago2, dcr-1, dcr-2, eef1alpha, act5c, clipb14, clipa8), \texttt{ceii\_v3.2} resolved the key coherence failures (strong in-domain genes no longer collapsed to zero; weak/non-significant genes remained low; near-boundary/out-of-domain genes abstained). Nonetheless, because held-out applicable reliability is not yet sufficiently stable for central probabilistic claims, cEII is retained as an auxiliary layer and removed from the main manuscript claim.
...
The revised inference layer explicitly separates three outputs: raw EII magnitude, calibrated cEII probability, and matched-neutral significance. Raw EII is a standardized dispersion effect size and remains a primary output. $q_{\mathrm{emp}}$ indicates excess branch--site dispersion relative to matched-neutral expectation and remains a primary inferential layer. cEII is intentionally nullable outside applicability support and is retained here as an auxiliary post-inference diagnostic rather than a central claim.
...
Several limitations should be noted. First, cEII is simulator-conditional; calibration quality depends on the realism and coverage of the benchmark domain. Second, site-level identifiability remained less stable than gene-level identifiability in OOD strata, so localization claims require stronger evidence and more conservative interpretation. Third, in the current benchmark instance, held-out applicable labels were sparse enough that probability reliability was not robust for central claims; this is why cEII is demoted from the manuscript headline. Finally, empirical application requires careful mapping of operational branch labels onto biological hypotheses and should be accompanied by classical branch--site analyses rather than replaced by BABAPPAi outputs alone.

\section{Conclusion}

BABAPPAi reframes branch--site analysis around recoverability and matched-neutral calibration while keeping a fixed frozen inference backbone. In this manuscript revision, the central, defensible outputs are raw dispersion diagnostics (raw EII), matched-neutral significance ($p_{\mathrm{emp}}, q_{\mathrm{emp}}$), and applicability-aware abstention. cEII remains implemented as an auxiliary calibration layer, but is not used as a main claim until broader held-out reliability support is established. BABAPPAi does not substitute for classical branch--site testing and does not claim proof of adaptive substitution.

\section*{Availability and implementation}

BABAPPAi is available as open-source software at \url{https://github.com/sinhakrishnendu/BABAPPAi} and as the installable package \texttt{babappai} (\cite{BABAPPAiRepo}). The release analyzed here reports raw EII diagnostics, matched-neutral significance outputs (\texttt{p\_emp}, \texttt{q\_emp}, \texttt{significant\_bool}), and applicability-aware abstention metadata as primary outputs; cEII outputs remain available as auxiliary applicability-gated diagnostics. The canonical frozen BABAPPA$\Omega$ model artifact is archived on Zenodo (\cite{BABAPPAOmegaModel}); the BABAPPAi software-release snapshot is archived on Zenodo (\cite{BABAPPAiZenodo}); and benchmark/reproducibility artifacts are archived on Zenodo (\cite{BABAPPAiBenchmarkArchive}).

\section*{Data availability}

All benchmark data used in this study were generated by simulation. Source code for BABAPPAi, including the manuscript-linked calibration, validation, and reporting scripts, is publicly available at GitHub (\cite{BABAPPAiRepo}). The canonical frozen BABAPPA$\Omega$ model artifact is publicly archived via Zenodo (\cite{BABAPPAOmegaModel}). The software-release record and reproducibility bundles (benchmark outputs, run manifests, and supporting artifacts) are publicly archived via Zenodo (\cite{BABAPPAiZenodo,BABAPPAiBenchmarkArchive}). The exact files used in this manuscript are included in the repository and supplementary package.
...
\bibitem{BABAPPAiRepo}
Sinha K.
BABAPPAi repository: framework code, scripts, and manuscript sources.
GitHub. \url{https://github.com/sinhakrishnendu/BABAPPAi}. Accessed 30 March 2026.

\bibitem{BABAPPAOmegaModel}
Sinha K.
BABAPPA$\Omega$ canonical frozen model artifact.
Zenodo. \url{https://doi.org/10.5281/zenodo.18195869} (2026).

\bibitem{BABAPPAiZenodo}
Sinha K.
BABAPPAi software-release provenance archive.
Zenodo. \url{https://doi.org/10.5281/zenodo.18520163} (2026).

\bibitem{BABAPPAiBenchmarkArchive}
Sinha K.
BABAPPAi benchmark and reproducibility artifact archive.
Zenodo. \url{https://doi.org/10.5281/zenodo.18197957} (2026).
