...
\begin{abstract}
Branch--site methods are widely used to infer episodic positive selection from coding-sequence alignments, but their conclusions depend on codon-model assumptions and on whether available data contain enough information to resolve lineage-restricted signal. Recombination, alignment uncertainty and heterogeneous background constraint can reduce recoverability even when episodic perturbation is biologically present. BABAPPAi is a likelihood-free framework that targets branch--site \emph{recoverability/identifiability} rather than direct adaptation testing. The method combines forward simulation under codon-level mutation--selection dynamics with amortized neural inference trained to recognize branch--site deviations from matched neutral background structure, without gene-wise likelihood fitting or direct estimation of $\omega$. BABAPPAi reports an Episodic Identifiability Index (EII) as a diagnostic magnitude summary and computes empirical Monte Carlo calibration p-values from matched neutral replicate exceedance, followed by Benjamini--Hochberg q-value adjustment across tested genes. In held-out synthetic benchmarks, predicted burden increased with simulated episodic load, remained unstructured under neutrality, and was more stable at gene level than at individual sites. Performance degraded gradually under recombination-aware segmentation, heterogeneous mutation rates and alignment noise, while deterministic execution and explicit input validation supported robust software behaviour. BABAPPAi is intended as a complement to classical branch--site tests: EII ranks recoverability magnitude, whereas empirical p/q values quantify excess dispersion relative to matched neutral calibration.
\end{abstract}
...
\begin{itemize}
    ...
    \item The software reports $\mathrm{EII}_{z}$ and $\mathrm{EII}_{01}=\sigma(\mathrm{EII}_{z})$ as recoverability diagnostics, and reports empirical calibration significance via \texttt{p\_emp}, \texttt{q\_emp}, and \texttt{significant\_bool}.
    \item EII bands are retained for descriptive reporting only; inferential significance is defined by \texttt{q\_emp} under matched neutral calibration.
\end{itemize}
...
For inferential calibration, BABAPPAi computes an empirical one-sided Monte Carlo p-value from matched neutral replicate dispersion values $D_0^{(1)},\ldots,D_0^{(M)}$:
\[
p_{\mathrm{emp}}=\frac{1+\sum_{m=1}^{M}\mathbf{1}\!\left[D_0^{(m)}\ge D_{\mathrm{obs}}\right]}{M+1}.
\]
Across genes in the same analysis set, multiple testing is controlled with Benjamini--Hochberg adjustment:
\[
q_{\mathrm{emp}}=\mathrm{BH}\!\left(p_{\mathrm{emp}}\right),
\]
and the default significance decision is
\[
\texttt{significant\_bool}=\left(q_{\mathrm{emp}}\le \alpha\right), \qquad \alpha=0.05.
\]

Operational EII reporting bands are retained as descriptive summaries:
\[
\begin{array}{ll}
...
\end{array}
\]
Legacy fields based on EII thresholds are retained for backward-compatible descriptive reporting only and are not used as the primary inferential rule.
...
Additional diagnostics included enrichment of true signal among the highest-scoring sites, bootstrap intervals for rank summaries, and a channel-collapse index assessing whether branch-specific outputs degenerated into nearly identical internal representations. Significance calibration analyses explicitly evaluated neutral $p_{\mathrm{emp}}$ behavior (histograms and QQ diagnostics), neutral significant rate at $q_{\mathrm{emp}}\le\alpha$, and q-based operating characteristics (FPR/TPR) versus descriptive EII-threshold baselines. To evaluate robustness, stress tests introduced recombination-aware segmentation, heterogeneous mutation rates, alignment noise and malformed input files.
...
\subsection{EII magnitude and empirical significance show complementary behavior}

EII increased with simulated episodic burden and remained near neutral baseline in negative-control datasets, supporting its role as a recoverability magnitude summary. Empirical calibration support was quantified separately by $p_{\mathrm{emp}}$ and $q_{\mathrm{emp}}$: genes with high episodic burden were enriched for low q-values, while neutral simulations were expected to show low significant rates at $q_{\mathrm{emp}}\le\alpha$ when calibration was adequate. Threshold-based EII calling was therefore treated as a descriptive comparator only, while inferential support was reported through the q-based decision layer.
...
The revised inference layer separates diagnostic magnitude from inferential support. EII remains useful because it quantifies recoverability magnitude on a continuous scale, but inferential support is now based on empirical calibration p-values and BH-adjusted q-values under matched neutral simulation. In this framing, significance indicates excess branch--site dispersion relative to matched neutral expectation. It is not equivalent to dN/dS estimation and does not by itself prove adaptive substitution.
...
Several limitations should be noted. First, the realism of inference depends on the realism of the simulator. Although the simulator incorporates mutation bias, codon-usage heterogeneity, branch-specific perturbation and local segmentation, it cannot capture every feature of real coding-sequence evolution. Second, empirical p-values are simulator-conditional; significance quality depends on the adequacy of matched neutral calibration. Third, numerical behavior of EII depends on the selected dispersion statistic and calibration dispersion scale, so EII bands should remain descriptive. Finally, the present manuscript is based on synthetic benchmarking; empirical application will require careful mapping of operational branch labels back onto biologically motivated phylogenetic hypotheses.

\section{Conclusion}

BABAPPAi reframes branch--site analysis around recoverability and matched-neutral calibration. By combining forward simulation under mutation--selection dynamics with likelihood-free neural inference, it diagnoses whether coding-sequence alignments contain enough structured variation for episodic interpretation and identifies the scale at which that structure becomes measurable. The framework reports EII as a diagnostic magnitude measure and empirical p/q calibration support as inferential evidence of excess dispersion relative to matched neutral expectation. It does not substitute for classical branch--site testing and does not claim proof of adaptive substitution.

\section*{Availability and implementation}

BABAPPAi is available as an open-source Python package at \url{https://github.com/krishnendusinha/babappai} and as the installable package \texttt{babappai} with CLI command \texttt{babappai}. The released software reports EII diagnostics together with empirical matched-neutral p-values and BH-adjusted q-values. BABAPPAi is the renamed continuation of the BABAPPA$\Omega$ codebase. Legacy frozen-model assets currently remain under the BABAPPA$\Omega$ naming for provenance and reproducibility; the frozen reference model used here is archived at \url{https://doi.org/10.5281/zenodo.18195869}. Legacy software-release provenance is archived at \url{https://doi.org/10.5281/zenodo.18520163}. Benchmarking artifacts, execution logs and related reproducibility materials are archived at \url{https://doi.org/10.5281/zenodo.18197957}.
