...
\begin{abstract}
Branch--site analyses are often interpreted as evidence for episodic positive selection, but a prior question is whether episodic structure is \emph{recoverable} from the available alignment. BABAPPAi is a likelihood-free framework that targets this identifiability question and separates it from adaptation claims. We lock the raw dispersion statistic as $D_{\mathrm{obs}}=\mathrm{Var}_{\mathrm{ddof}=1}(\overline{\ell}_i)$, where $\overline{\ell}_i$ is the site-level mean branch logit, and define raw $ \mathrm{EII}_z=(D_{\mathrm{obs}}-\mu_0)/\sigma_0 $ under matched-neutral Monte Carlo calibration. We then calibrate raw EII on held-out simulator truth to obtain calibrated identifiability probabilities: $c\mathrm{EII}_{\mathrm{gene}}=P(I_{\mathrm{gene}}=1\mid \text{data})$ and $c\mathrm{EII}_{\mathrm{site}}=P(I_{\mathrm{site}}=1\mid \text{data})$. Recoverability targets are truth-aware composites of site enrichment, site/branch rank agreement, burden alignment, and replicate-stability terms, with pre-registered binary labels for gene- and site-scale identifiability. Calibration uses isotonic regression on a dedicated calibration split with separate test and regime-holdout (OOD) evaluation. Matched-neutral significance remains a distinct inferential layer via empirical $p_{\mathrm{emp}}$ and BH-adjusted $q_{\mathrm{emp}}$. In the current locked benchmark, gene-scale calibration was more stable than site-scale calibration, and OOD degradation was strongest for site localization under high nuisance regimes. BABAPPAi is therefore presented as a calibrated identifiability diagnostic that complements, but does not replace, classical branch--site hypothesis tests.
\end{abstract}
...
\begin{itemize}
    ...
    \item The software reports raw dispersion diagnostics ($\mathrm{EII}_{z,\mathrm{raw}}$, $\mathrm{EII}_{01,\mathrm{raw}}$) and calibrated identifiability probabilities ($c\mathrm{EII}_{\mathrm{gene}}$, $c\mathrm{EII}_{\mathrm{site}}$).
...
\[
D_{\mathrm{obs}}=\mathrm{Var}_{\mathrm{ddof}=1}\!\left(\overline{\ell}_i\right), \qquad
\overline{\ell}_i=\frac{1}{|\mathcal{B}|}\sum_{b\in\mathcal{B}}\ell_{i,b},
\]
where $\ell_{i,b}$ is the model logit for site $i$ and branch $b$. This removes prior ambiguity about equivalent dispersion summaries.
...
Empirical calibration maps raw EII to calibrated identifiability probability via isotonic regression on a dedicated calibration split:
\[
c\mathrm{EII}_{\mathrm{gene}}=P(I_{\mathrm{gene}}=1\mid \mathrm{EII}_{z,\mathrm{raw}},X),\qquad
c\mathrm{EII}_{\mathrm{site}}=P(I_{\mathrm{site}}=1\mid \mathrm{EII}_{z,\mathrm{raw}},X),
\]
where $X$ includes alignment covariates when available (taxa count, alignment length, nuisance bins). Bootstrap resampling provides calibration-curve uncertainty.

Calibrated decision bands are therefore empirical and versioned (\texttt{calibration\_version}); they are not fixed constants.
...
Symbol & Definition \\
\hline
$D_{\mathrm{obs}}$ & $\mathrm{Var}_{\mathrm{ddof}=1}(\overline{\ell}_i)$ across codon sites (locked). \\
...
$c\mathrm{EII}_{\mathrm{gene}},c\mathrm{EII}_{\mathrm{site}}$ & Calibrated $P(I=1\mid\text{data})$ from held-out calibration. \\
...
The locked benchmark used 96 simulated genes (24 per latent regime), with strict split separation at scenario level: train $n=52$, calibration $n=16$, test $n=16$, OOD $n=12$. OOD scenarios were predefined nuisance-heavy conditions (deep trees with high recombination or high alignment noise).

\subsection{Raw EII versus calibrated cEII}

Raw EII remained a useful continuous dispersion score but was not directly interpretable as identifiability probability. Isotonic calibration produced versioned mappings from raw $\mathrm{EII}_{z,\mathrm{raw}}$ to $c\mathrm{EII}_{\mathrm{gene}}$ and $c\mathrm{EII}_{\mathrm{site}}$. Under the current benchmark, the empirical decision thresholds selected from calibration data were $0.625$ (gene) and $0.167$ (site), with calibrated class edges derived from the same held-out mapping.

\subsection{Reliability and operating characteristics}

On held-out test data, gene-scale calibration yielded Brier $=0.281$ and ECE $=0.188$; OOD values were Brier $=0.266$ and ECE $=0.125$. Site-scale calibration was less stable (test: Brier $=0.233$, ECE $=0.241$; OOD: Brier $=0.153$, ECE $=0.185$), with marked PPV/FDR degradation in low-prevalence strata. These quantitative results support the intended interpretation: gene-level identifiability can emerge before site-level localization becomes decision-stable.
...
BABAPPAi is available as an open-source Python package at \url{https://github.com/krishnendusinha/babappai} and as the installable package \texttt{babappai} with CLI command \texttt{babappai}. The released software reports raw EII diagnostics, calibrated cEII probabilities/classes, and empirical matched-neutral p/q significance outputs with calibration/model provenance fields. BABAPPAi is the renamed continuation of the BABAPPA$\Omega$ codebase. Legacy frozen-model assets currently remain under the BABAPPA$\Omega$ naming for provenance and reproducibility; the frozen reference model used here is archived at \url{https://doi.org/10.5281/zenodo.18195869}. Legacy software-release provenance is archived at \url{https://doi.org/10.5281/zenodo.18520163}. Benchmarking artifacts, execution logs and related reproducibility materials are archived at \url{https://doi.org/10.5281/zenodo.18197957}.
